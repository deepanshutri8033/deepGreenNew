...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Add click event listener to menu toggle button if present
  if (menuToggleBtn) {
  ...
  if (themeToggleBtn) {
    ...
    // Set initial text label based on active class
    const isCurrentlyDark = document.documentElement.classList.contains("dark-theme");
    if (themeText) {
      themeText.innerText = isCurrentlyDark ? "Theme: Dark" : "Theme: Light";
    }
    ...
    themeToggleBtn.addEventListener("click", () => {
      ...
      if (themeText) {
        themeText.innerText = isDark ? "Theme: Dark" : "Theme: Light";
      }
    });
  }
});